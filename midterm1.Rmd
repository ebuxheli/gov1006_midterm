...
```{r figure2}
# NOTE: The below is a failed attempt to build a plot with a 95% confidence
# interval. I am including this in the code because I believe I was close to
# being able to reproduce the confidence interval in the paper but ran into some
# technical troubles along the way. I also put a lot of work into trying to
# figure this out and thought it'd be best to show that rather than delete all
# of this and make it seem like I just copped out.
...
group_by(anxcond3) %>%
 ...
 ggplot() +
   ...
   geom_errorbar(aes(x = type, ymin = lower_conf, ymax = upper_conf), position = "dodge", width = 0.25) +
...
filter(data, !is.na(anxcond3), !is.na(SCDBradVidManipAll_mean)) %>%
  ggplot(aes(x = factor(anxcond3), y = SCDBradVidManipAll_mean)) +
  geom_boxplot() +
  coord_flip() +
  theme_light() +
  scale_y_continuous(limits = c(-0.5, 1.5)) +
